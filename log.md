...
> **덧(2026-09-05) 재측정 — 실패하는 것은 `curl` 뿐이다.**
>
> ```
> 브라우저 fetch  → /health 200 "ok", /admin/lock 200 {"locked":...}
> 브라우저 WebSocket → 방 개통 101
> Node fetch    → /health 200 "ok"
> curl          → 타임아웃 (rc=28)
> DNS·TCP 443   → 정상 (172.67.185.176 / 104.21.19.86)
> ```
>
> 따라서 **관리자 기능도 이 노트북에서 된다.** 다른 망으로 옮겨갈 필요 없다.
> 2026-09-04 에 "브라우저·Node fetch 도 전부 실패한다" 고 적었던 것은 취소한다 —
> 한 번 난 타임아웃을 결론으로 삼은 것이었다.
>
> `node fetch` 로 `/admin/lock` 을 부르면 `403` 이 나는데 이건 도달 실패가 아니라
> **오리진 검사 결과**다. 서버까지 갔다는 뜻이니 오히려 정상 신호로 읽으면 된다.
...
**5. "접속이 안 된다" 면 `wrangler tail` 부터 켤 것**
```bash
cd apps/server && npx wrangler tail fox-server --format json
```
요청마다 `origin` 과 `response.status` 가 그대로 찍힌다. 아무것도 안 찍히면 그 기기의
망 문제, `403` 이면 오리진, `503` 이면 관리자 차단, `101` 이면 정상이다.
Cloudflare API 로 붙으므로 **내 기기가 서버에 못 닿아도 동작한다.**

방금 이관 직후의 장애도 이걸로 30초 만에 갈랐다(6절). 그 전까지는 DNS·TLS·SNI 를
뒤지고 있었다. **응답 코드를 볼 수 있는데 추측하지 말 것.**
...
**2026-09-05 재확인 — 이 PC 에서 관리자 기능이 안 된다는 것도 오진이었다**
`curl` 은 여전히 실패하지만 **`node fetch` 와 브라우저는 `/health` 200 을 받는다.**
따라서 **관리자 모드 조작을 이 노트북에서 해도 된다.** 휴대폰 데이터로 옮겨갈 필요가 없다.

같은 진단 중에 `node fetch` 가 한 번 타임아웃나서 "node 도 막혔다" 고 문서에 적었다가,
다시 재보고 되돌렸다. **한 번의 실패로 결론 내리지 말 것** — 이 프로젝트에서 같은 실수를
`curl` 로 한 번, `node` 로 또 한 번 했다.

---

## 6. Netlify → Cloudflare Pages 이관 (2026-09-05)
...
이때 곁가지로 정정된 것이 하나 더 있다. 5절에 적었다.
