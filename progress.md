...
>
> **이것 때문에 실제로 한 번 막혔다.** 지난 수업 끝나고 켜 둔 차단이 그대로 남아
> 다음 접속이 전부 503 이었다. 서버·주소·배포를 한참 뒤졌지만 원인은 이 버튼이었다.
> **"안 된다" 는 말을 들으면 여기부터 본다.** (log.md 6절)
>
> 관리자 기능은 **이 노트북에서도 된다.** 브라우저에서 `/admin/lock` 이 200 으로 응답한다.
...
> 네트워크 차단이 아니다 (log.md 참고). `node -e "fetch(...)"` 와 브라우저는 정상이다.
>
> `/admin/lock` 은 오리진을 보므로 **브라우저 콘솔**로 확인한다. node 로는 403 이 난다.
...
`tail` 은 Cloudflare API 로 붙는다. 그래서 **내 기기에서 서버에 못 닿는 상황에서도**
서버가 무엇을 받고 무엇을 돌려주는지 그대로 볼 수 있다.
